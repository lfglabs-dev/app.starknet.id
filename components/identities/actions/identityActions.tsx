...
const IdentityActions: FunctionComponent<IdentityActionsProps> = ({identity, tokenId, isIdentityADomain, isOwner}) => {
   ...
   const nextAutoRenew = useMemo(() => {
      ...
      if (identity?.domainExpiry) {
         if (identity?.domainExpiry + monthInSeconds < now) {
            ...
            return "Next payment on " + timestampToReadableDate(identity?.domainExpiry - monthInSeconds);
         }
      }
   }, [identity]);
   ...
   const {sendAsync: setMainId, data: mainDomainData} = useSendTransaction({
      calls: identity ? identityChangeCalls.setAsMainId(identity, hasReverseAddressRecord, callDataEncodedDomain) : [],
   });
   ...
   useEffect(() => {
      ...
      fetch(`${process.env.NEXT_PUBLIC_SERVER_LINK}/renewal/get_renewal_data?addr=${hexToDecimal(address)}&domain=${identity.domain}`)
         .then((response) => response.json())
         .then((data) => {
         ...
         autoRenewalData.forEach((renewalData) => {
            disableCallData.push(
               autoRenewalCalls.disableRenewal(
                  ...
                  renewalData.auto_renew_contract ?? (process.env.NEXT_PUBLIC_RENEWAL_CONTRACT as string),
   ...
   return (
      <div className={styles.actionsContainer}>
         <>
               ...
               {identity && (!isOwner || !address) && isIdentityADomain && (
                  <>
                     <ClickableAction
                        ...
                        icon={<UnframedIcon width="30" color={theme.palette.secondary.main} />}
                        description="Check this identity on Unframed"
                        onClick={() => window.open(`https://unframed.co/item/${process.env.NEXT_PUBLIC_IDENTITY_CONTRACT}/${tokenId}`)}
                     />
                     <ClickableAction
                        title="View on Pyramid"
                        icon={<PyramidIcon width="25" color={theme.palette.secondary.main} />}
                        description="Check this identity on Pyramid"
                        onClick={() => window.open(`https://pyramid.market/collection/${process.env.NEXT_PUBLIC_IDENTITY_CONTRACT}/${tokenId}`)}
                     />
                  </>
               )}
                  ...
                  <div className={styles.identityActions}>
                     {callDataEncodedDomain[0] === "1" ? (
                        <ClickableAction
                           ...
                           style="primary"
                           description={`Will expire on ${timestampToReadableDate(identity?.domainExpiry ?? 0)}`}
                           icon={<RenewalIcon width="18" color={theme.palette.primary.main} />}
                     ...
                     <ClickableAction
                        ...
                        icon={<SignsIcon width="23" color={theme.palette.secondary.main} />}
                     ...
                     {viewMoreClicked ? (
                        <>
                           <ClickableAction
                              ...
                              icon={<TransferIcon width="20" color={theme.palette.secondary.main} />}
                              onClick={() => setIsTransferFormOpen(true)}
                           />
                           <ClickableAction
                              title="CREATE A SUBDOMAIN"
                              description="Create a new subdomain"
                              icon={<PlusIcon width="18" color={theme.palette.secondary.main} />}
                           ...
                           {!isMainDomain && (
                              <ClickableAction
                                 ...
                                 icon={<MainIcon width="21" firstColor={theme.palette.secondary.main} secondColor={theme.palette.secondary.main} />}
                                 onClick={() => setMainId()}
                              />
                           )}
            ...
            />
            <TransferFormModal identity={identity} handleClose={() => setIsTransferFormOpen(false)} isModalOpen={isTransferFormOpen} />
            <SubdomainModal
            ...
            />
            <TxConfirmationModal txHash={txHash} isTxModalOpen={isTxModalOpen} closeModal={() => setIsTxModalOpen(false)} title="Your Transaction is on it's way !" />
         </>
      </div>
   );
};
