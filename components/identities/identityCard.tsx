...
import {convertNumberToFixedLengthString, minifyAddress, shortenDomain} from "../../utils/stringService";
...
const IdentityCard: FunctionComponent<IdentityCardProps> = ({tokenId, identity, isOwner, onPPClick, ppImageUrl}) => {
   const responsiveDomainOrId = identity?.domain ? shortenDomain(identity.domain, 25) : `SID: ${tokenId}`;
         ...
         <div className={styles.container}>
            <div className="lg:mt-10 flex items-center lg:justify-between justify-center sm:text-center gap-3 sm:gap-5 my-2 flex-wrap lg:flex-row sm:flex-col">
               <div className="my-2 text-center">
                  <div className={styles.pfpSection} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                     {isHovered && isOwner && !isMobile ? (
                        ...
                        <>
                           {ppImageUrl ? (
                              <img src={ppImageUrl} height={170} width={170} alt="identicon" className={styles.pfpImg} />
                           ) : (
                              <Skeleton variant="rectangular" width={170} height={170} className={styles.pfpImg} />
                           )}
                           {isOwner && isMobile ? (
                  ...
                  {identity?.domainExpiry ? (
                     <Tooltip title="Expiry date of this domain" arrow>
                        <div className={styles.expiryContainer}>
                           ...
                           <p className={styles.expiryText}>{timestampToReadableDate(identity.domainExpiry)}</p>
                        </div>
                     </Tooltip>
                  ) : null}
               </div>
               {minting ? (
                  <div className="text-left h-full py-2">
                     <h1 className="text-3xl font-bold font-quickZap">Minting your identity...</h1>
               ...
               <div className="">
                  <div className="lg:ml-8 sm:ml-0 sm:justify-center">
                                    ...
                                    <>
                                       ...
                                       {identity?.domain ? (
                                          <div className={styles.addressBar}>
                                             ...
                                             <CopyContent value={identity?.targetAddress} className="cursor-pointer ml-3" />
                                          </div>
                                       ) : null}
                                    </>
                                 ) : null}
                     ...
                     <SocialMediaActions identity={identity} isOwner={isOwner} tokenId={tokenId} />
                  </div>

                  <img alt="leaf" src="/leaves/new/leaf02.webp" className={styles.lg1} />
                  <img alt="leaf" src="/leaves/new/leaf01.webp" className={styles.lg2} />
                  <img alt="logo" src="/visuals/detoured_logo.svg" className={styles.detouredLogo} />
                  <img alt="logo" src="/visuals/text.svg" className={styles.detouredTextLogo} />
               </div>
            </div>
         </div>
         <div className={styles.cardCode}>
            <p>{convertNumberToFixedLengthString(tokenId)}</p>
            <svg width="300" height="6" viewBox="0 0 380 6" fill="none" xmlns="http://www.w3.org/2000/svg">
