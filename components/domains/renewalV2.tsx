...
const RenewalV2: FunctionComponent = () => {
   ...
   useEffect(() => {
      ...
      starknetIdNavigator?.getProfileData(address as string, false).then((res) => {
   ...
   return (
      <>
         <div className="w-full flex flex-col xl:flex-row justify-center gap-4 px-3 py-10 lg:px-32 md:px-16 sm:pb-12 xl:min-h-[88vh] ">
            <aside className={`${styles.purchaseStepNav}`} role="navigation">
               <RegisterSteps currentStep={currentStep} setStep={goToStep} showPfp={showPfp} isLoading={isLoading} />
            ...
            <div className="flex-1 w-full xl:w-[932px] xl:min-w-[932px] border-solid ">
               {currentStep === 1 && <UserInfoForm type={FormType.RENEW} goToNextStep={goToNextStep} />}
               {currentStep === 2 && <SelectPfp goToNextStep={goToNextStep} />}
               {currentStep === 3 && <CheckoutCard type={FormType.RENEW} discount={evergreenDiscounts.renewal} />}
            </div>
         </div>
      </>
   );
