...
const UserInfoForm: FunctionComponent<UserInfoFormProps> = ({type, goToNextStep}) => {
   ...
   function changeDuration(type: IncrementType, value: number = formState.durationInYears): void {
      let newValue = value;
      ...
      if (isNaN(newValue) || newValue > maxYearsToRegister || newValue < 1) return;
      updateFormState({
   ...
   const getButtonText = (): string => {
      return type === FormType.RENEW && !areDomainSelected(formState.selectedDomains) ? "Select a domain to renew" : "Next step";
   };

   const isDisabled = (): boolean => {
      return !formState.durationInYears || formState.durationInYears < 1 || (type === FormType.RENEW && !areDomainSelected(formState.selectedDomains));
   };
         ...
         <div className={styles.card}>
            ...
            <div className={`${styles.form}`}>
               ...
               <div className="flex flex-col items-start gap-6 self-stretch">
                  ...
                  />
                  {type === FormType.REGISTER ? <SelectIdentity tokenId={formState.tokenId} changeTokenId={changeTokenId} /> : null}
                  <NumberTextField
                     value={formState.durationInYears}
                     label="Years to register (max 25 years)"
                     placeholder="years"
                     onChange={(e) => changeDuration(IncrementType.SET, Number(e.target.value))}
